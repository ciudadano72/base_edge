...
class _cell_(object):
    def __init__(
        ...
        denLayer0,denLayer1_0,denLayer1_1,denLayer2,limit,absRnA
        ):
        self.__Patternsrtn=[];self.__RnA=[];self.__Wij_neurons=[];self.__epoc=int(epoc)
        self.__adj_0=int(adj0);self.__adj_1=int(adj1);self.__corr=float(corr);self.__limit=float(limit);
        self.__denLayer_0=int(denLayer0);self.__denLayer_1_0=int(denLayer1_0);
        self.__denLayer_1_1=int(denLayer1_1);self.__denLayer_2=int(denLayer2);
        self.__dinWij=int(dimWij);self.__neurons=int(neurons);
        self.__id_ = int(id_);self.__ref_class = (self.__class__.
        __name__+"_" + str(self.__id_));self.__domains=absRnA
        
        for x in range (int(neurons)):
            (self.__RnA.append(NaSS.
            _n_(x,int(dimNaSS))))

    def netlist(self,input_):
                  vector=[]
                  ...
                  for layer_1 in range(self.__denLayer_1_0,self.__denLayer_1_1):
                      self.__RnA[layer_1].ibn_(vector)
                      
                  for rst in vector[:]:
                      vector.remove(rst)
                      
                  for x in range(self.__denLayer_1_0,self.__denLayer_1_1):
                      vector.append(self.__RnA[x]._out_)
