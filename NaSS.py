...
class _n_(object):
    def __init__(self,id_,dim):
        self._learn=0;self._out_ = 0.0;self.__dimension=dim
        self._Wij=[];self.__selector=0;self._SuM_DenWij=0;self.__id_ = int(id_)
    ...
    def setWij(self,Wij):
        ...
        for x in range(int(self.__dimension)):
            self._Wij.append(Wij[x])
            
    def func_a(self):
        for i in range(len(self.__vector_neuron)):
            self._SuM_DenWij = (self._SuM_DenWij+
            (self.__vector_neuron[i]*self.
                    _Wij[i]))
    ...
    def ibn_(self,tensor_den):
        ...
        if self._learn==0:
            ...
            elif self.__selector==1:self.func_a()
        elif self._learn==1:self.func_a()
